...
class MovingObject(AbstractObject):
    ...
    def get_action_according_to_plan(self):
        ...
        if current_movement[0] == 'l':
            ...
            destination = np.array([current_movement[1], current_movement[2]], dtype=np.int32)
            sample_radius = 200
    ...
    def path_v3(self, direction, speed, mountain_inner_range=140, mountain_outer_range=160):
        ...
        new_location = np.round([self.location[0] + step[0], self.location[1] + step[1]]).astype(np.int32)
        ...
        if self.terrain.violate_edge_constraints(self.location[0], self.location[1], 1, 1):
            self.location = np.clip(self.location, [0, 0], [self.terrain.dim_x - 1, self.terrain.dim_y - 1]).astype(np.int32)
    ...
    def plan_path_to_intercept(self, speed, direction, current_loc):
        ...
        if np.abs(direction) < 1e-3:
            ...
            x_tangent, y_tangent = np.round(x_tangent).astype(np.int32), np.round(y_tangent).astype(np.int32)
        ...
        if is_blue_on_fugitive_movement_side:
            if self.terrain.violate_edge_constraints(x_tangent, y_tangent, 1, 1):
                ...
                x_tangent = np.clip(x_tangent, 0, self.terrain.dim_x - 1).astype(np.int32)
                y_tangent = np.clip(y_tangent, 0, self.terrain.dim_y - 1).astype(np.int32)
